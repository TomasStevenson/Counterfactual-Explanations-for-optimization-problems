...
sol=dec._solve_dispatch_lp(b_bs,uk,T,True,p_init)  # exact dispatch duals at b_BS
...
nL=len(DATA.lines); exmax=0.0; nex=0
for ell in free:
    for t in range(T):
        for tag,arr in (("mup",sol["mu_p"]),("mum",sol["mu_m"])):
            v=m.getVarByName(f"{tag}_0[{ell},{t}]")
            if v is None: continue
            mu_exact=float(arr[ell,t]); ub=float(v.UB)
            if mu_exact > ub + 1e-9:
                nex+=1; exmax=max(exmax, mu_exact-ub)
                if nex<=8: print(f"  EXCLUDED {tag}[{ell},{t}]: mu_exact={mu_exact:.4f} > UB={ub:.4f}  (by {mu_exact-ub:.4f})")
print(f"TOTAL mu excluded at b_BS: {nex}  worst overshoot {exmax:.4f}")
